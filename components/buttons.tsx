...
export default function ButtonDefault(props: { text: string, link: string }){
    return(
        <Link href={props.link} className="border-2 border-white bg-white text-primary py-2 px-6 rounded hover:bg-transparent hover:text-white  delay-150 duration-300 ease-in-out">
        <button>{props.text}</button>
        </Link>
    )
}